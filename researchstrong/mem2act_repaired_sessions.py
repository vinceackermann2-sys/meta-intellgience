...
OUT=Path(__file__).with_name('mem2act_repaired_sessions_report.json')
...
def normalize_upstream(source_id,src,row):
    ...
    # Stamp provenance exactly from the upstream conversation identity; do not infer cross-source time.
    turns2=[]
    for t in turns:
        z=dict(t) if isinstance(t,dict) else {'role':'unknown','content':str(t)}
        z.setdefault('source_id',source_id);turns2.append(z)
    return {'session_id':f'repaired::{source_id}','original_conversation_ids':[source_id],'turns':turns2,'turn_count':len(turns2),'has_tool_calls':any(bool(t.get('tool_calls')) for t in turns2 if isinstance(t,dict)),'repair_source':src,'repair_mode':'exact_upstream_conversation'}

def build():
    # Project away all labels immediately. No tool_call, grounding_info, evolution_chain,
    # answer, or gold value is retained or inspected for context repair.
    qproj=[qa_projection(q) for q in stream(BASE+'Mem2ActBench/qa_dataset.jsonl')]
    sessions,by=released_index();up=upstream_rows();repaired={};stats=defaultdict(int);missing_upstream=[]
    for q in qproj:
        qid=q['qa_id'];ids=q['source_conversation_ids'];s=resolve_released(ids,sessions,by)
        if s is not None:
        ...
        if rows:
            turns=[];sources=[]
            for r in rows:turns.extend(r['turns']);sources.extend(r['original_conversation_ids'])
