...
def country_variants(s):
    out=[]
    try:
        c=pycountry.countries.lookup(str(s).strip());out.extend([c.name,c.alpha_2,c.alpha_3])
...
def language_variants(s):
    out=[]
    try:
        x=pycountry.languages.lookup(str(s).strip())
...
def date_variants(s):
    ...
    try:
        ...
        for f in ['%Y-%m-%d','%m/%d/%Y','%d/%m/%Y','%m-%d-%Y','%d-%m-%Y','%Y/%m/%d']:out.append(d.strftime(f))
    except:pass
    return out
...
def string_variants(s):
    q=str(s).strip();out=[]
    if not q:return out
    out.extend([q.lower(),q.upper(),q.title()]);words=re.findall(r'[A-Za-z]+',q)
...
def variants(v):
    out=[]
    if isinstance(v,bool):out.extend([(str(v).lower(),'boolean'),('yes' if v else 'no','boolean')])
    if not isinstance(v,(dict,list)):
        q=str(v)
        out.extend((x,'string') for x in string_variants(q));out.extend((x,'country') for x in country_variants(q));out.extend((x,'language') for x in language_variants(q));out.extend((x,'color') for x in color_variants(q));out.extend((x,'number') for x in number_variants(q));out.extend((x,'date') for x in date_variants(q))
    return out
